...
def ensure_dirs() -> None:
    for directory in (OUTPUT_DIR, FIGURE_DIR, PREPROCESS_DIR, PNG_DIR, REPORT_DIR):
...
def save_figure(fig: plt.Figure, pdf_name: str) -> None:
    """同步保存论文用矢量 PDF 与便于预览的 300 dpi PNG。"""
    pdf_path = FIGURE_DIR / pdf_name
    png_path = PNG_DIR / f"{Path(pdf_name).stem}.png"
    fig.tight_layout()
...
def export_preprocessing_artifacts(df: pd.DataFrame, quality: dict[str, Any]) -> None:
    """导出当前统一口径的完整预处理数据与两张数据质量图。"""
    csv_path = PREPROCESS_DIR / "高血脂症数据_预处理完成.csv"
    xlsx_path = PREPROCESS_DIR / "高血脂症数据_预处理完成.xlsx"
    df.to_csv(csv_path, index=False, encoding="utf-8-sig")
    df.to_excel(xlsx_path, index=False, sheet_name="预处理数据")

    core_cols = [
        "痰湿质积分", "ADL评分", "IADL评分", "活动量表总分",
    ...
    sns.heatmap(
        corr, cmap="RdBu_r", center=0, vmin=-1, vmax=1, annot=True, fmt=".2f",
        square=True, linewidths=0.5, cbar_kws={"label": "Pearson 相关系数"}, ax=ax,
    ...
    save_figure(fig, "预处理_核心指标相关性热力图.pdf")

    standardized = (df[core_cols] - df[core_cols].mean()) / df[core_cols].std(ddof=0)
    long_data = standardized.melt(var_name="核心指标", value_name="标准化值")
    fig, ax = plt.subplots(figsize=(14.2, 7.2))
    sns.boxplot(
        data=long_data, x="核心指标", y="标准化值", color="#76a5d7",
        width=0.62, fliersize=1.8, linewidth=1.0, ax=ax,
    )
    ax.axhline(0, color="#555555", linewidth=0.8, linestyle="--")
    ax.set_xlabel("")
    ax.set_ylabel("标准化值（Z-score）")
    ax.tick_params(axis="x", rotation=35, labelsize=9)
    ax.grid(axis="x", visible=False)
    save_figure(fig, "预处理_核心指标标准化箱线图.pdf")

    quality["preprocessed_csv"] = str(csv_path.relative_to(ROOT)).replace("\\", "/")
    quality["preprocessed_xlsx"] = str(xlsx_path.relative_to(ROOT)).replace("\\", "/")
...
def load_and_preprocess() -> tuple[pd.DataFrame, dict[str, Any]]:
    ...
    export_preprocessing_artifacts(df, quality)
...
def write_results_report(
    ...
    lines = [
        ...
        "当前口径的完整预处理数据同时导出为 CSV 与 XLSX；核心指标相关性热力图和标准化箱线图同步保存 PDF 与 PNG。",
    ...
    lines += [
        ...
        "- 所有论文引用数值均保存为 CSV/JSON；图表同步输出论文用矢量 PDF 与预览用 300 dpi PNG。",
        "",
        "## 可复现运行方式",
        "",
        "```powershell",
...
def validate_results(
    ...
    xlsx_df = pd.read_excel(xlsx_path, sheet_name="预处理数据")
    assert len(csv_df) == len(xlsx_df) == len(df) == 1000
    assert list(csv_df.columns) == list(xlsx_df.columns) == list(df.columns)
    assert int(csv_df.isna().sum().sum()) == int(xlsx_df.isna().sum().sum()) == 0
    pd.testing.assert_frame_equal(csv_df, xlsx_df, check_dtype=False, rtol=1e-10, atol=1e-12)
    exported_rule = (
        (csv_df["总胆固醇_TC"] > 6.2)
        | (csv_df["甘油三酯_TG"] > 1.7)
        | (csv_df["低密度脂蛋白_LDL_C"] > 3.1)
        | (csv_df["高密度脂蛋白_HDL_C"] < 1.04)
    ).astype(int)
    assert float((exported_rule == csv_df["高血脂症标签"]).mean()) == 1.0
    checks.append("预处理：CSV/XLSX行列、缺失值、派生标签及诊断规则一致性通过")
    diagnostic_features = {
        "总胆固醇_TC", "甘油三酯_TG", "低密度脂蛋白_LDL_C", "高密度脂蛋白_HDL_C",
    }
    ...
    pdf_stems = {path.stem for path in FIGURE_DIR.glob("*.pdf") if path.stat().st_size > 0}
    png_stems = {path.stem for path in PNG_DIR.glob("*.png") if path.stat().st_size > 0}
    assert pdf_stems == png_stems
    assert len(pdf_stems) == 12
    assert {"预处理_核心指标相关性热力图", "预处理_核心指标标准化箱线图"} <= pdf_stems
    checks.append("图表：12张正式图的PDF与PNG成对存在且文件非空")
